...
CenteringTextElement.prototype.addText = function (text, prefix, suffix) {
  if (text) {
    ...
    for (var i = 0; i < splittedText.length; i++) {
      this.addTextline(splittedText[i], this.CSS_CLASSES.default, prefix, suffix);
    }
  }
};
...
CenteringTextElement.prototype.addSubText = function (text) {
  if (text) {
    var splittedText = text.split(/\s+/)
    for (var i = 0; i < splittedText.length; i++) {
      this.addTextline(splittedText[i], this.CSS_CLASSES.subtext, "(", ")");
    }
  }
};
...
CenteringTextElement.prototype.addEquivalents = function (text) {
  if (text) {
    var splittedText = text.split(/\s+/)
    for (var i = 0; i < splittedText.length; i++) {
      this.addTextline(splittedText[i], this.CSS_CLASSES.default);
    }
  }
};
