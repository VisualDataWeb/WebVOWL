...
AbsoluteTextElement.prototype.addText = function (text, yShift, prefix, suffix) {
  if (text) {
    ...
    for (var i = 0; i < splittedText.length; i++) {
      this.addTextline(splittedText[i], this.CSS_CLASSES.default, yShift, prefix, suffix);
    }
  }
};
...
AbsoluteTextElement.prototype.addSubText = function (text, yShift) {
  if (text) {
    var splittedText = text.split(/\s+/)
    for (var i = 0; i < splittedText.length; i++) {
      this.addTextline(splittedText[i], this.CSS_CLASSES.subtext, yShift, "(", ")");
    }
  }
};
...
AbsoluteTextElement.prototype.addEquivalents = function (text, yShift) {
  if (text) {
    var splittedText = text.split(/\s+/)
    for (var i = 0; i < splittedText.length; i++) {
      this.addTextline(splittedText[i], this.CSS_CLASSES.default, yShift);
    }
  }
};
...
AbsoluteTextElement.prototype.addTextline = function (text, style, yShift, prefix, postfix) {
  var tspan = this._textBlock().append("tspan")
    ...
    .classed(style, true)
    .text(this._applyPreAndPostFix(text, prefix, postfix))
    .attr("x", 0);
